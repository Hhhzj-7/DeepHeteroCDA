...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, noutput, dropout, negative_slope, nheads):
        ...
        self.attentions1 = GATConv(nhid*nheads, nhid, nheads, True, negative_slope=negative_slope, dropout=self.dropout)
        self.attentions2 = GATConv(nhid*nheads, nhid, nheads, True, negative_slope=negative_slope, dropout=self.dropout)
        self.attentions3 = GATConv(nhid*nheads, nhid, nheads, True, negative_slope=negative_slope, dropout=self.dropout)
...
class CDA_Decoder(nn.Module):
    ...
    def forward(self, epoch, CircRNAs, Rna_3, Drugs, Drugs_3, nodes_features, circRNA_index, drug_index):
        ...
        pair_nodes_features0 = torch.cat((circRNA_features, drug_features), 1) # (6612, 2*(489 + 256))
